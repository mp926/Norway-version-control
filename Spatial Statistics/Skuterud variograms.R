...
vK[[i]]<-variogram(Ks~1,df.adj[[i]], width=13, cutoff=60)
...
vK.fit[[i]]<-fit.variogram(vK[[i]], vgm(psill=var(df.adj[[i]]$Ks), "Exp", range=1,
...
vmodel.linesK[[i]]<-variogramLine(vK.fit[[i]],maxdist=60,n=200)
}
...
vK.orig=variogram(Ks~1,df, width=13, cutoff=60) #cutoff = distance where np first decreases
...
plot(vK.orig$dist,vK.orig$gamma, xlim=c(0,60), ylim=c(350000,700000), pch=16,
